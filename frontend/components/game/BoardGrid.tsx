...
export default function BoardGrid({
  ...
  const getDevelopmentLevel = (propertyId: number) =>
    game_properties.find((gp) => gp.property_id === propertyId)?.development ?? 0;

  const isPropertyMortgaged = (propertyId: number) =>
  ...
  return (
    <div className="grid grid-cols-11 grid-rows-11 w-full h-full gap-[2px] box-border select-none">
      ...
      <div
        className="
          col-start-2 col-span-9 row-start-2 row-span-9 
          bg-[#010F10]/90 flex flex-col justify-center items-center 
          p-4 md:p-6 lg:p-8 relative overflow-hidden rounded-xl 
          border border-cyan-900/30 pointer-events-none
        "
      />

      {properties.map((square) => {
        const gridRow = (square as any).grid_row ?? 1;
        ...
        return (
          <motion.div
            ...
            className="
              w-full h-full p-[2px] relative box-border group
              hover:z-[50] transition-transform duration-200
            "
          ...
          >
            <div
              className={`
                ...
                group-hover:scale-[2.1] group-hover:z-[100]           /* ← high z-index + scale */
                ${gridRow === 1
                  ? "origin-top group-hover:origin-bottom group-hover:translate-y-[80px]"
                  : "origin-bottom group-hover:origin-top group-hover:-translate-y-[80px]"}
                group-hover:shadow-2xl group-hover:shadow-cyan-600/60
                rounded-lg overflow-visible                       /* ← crucial fix */
            ...
            >
              ...
              {square.type === "property" && devLevel > 0 && (
                <div className="absolute top-1.5 right-1.5 bg-gradient-to-br from-yellow-400 to-amber-600 text-black text-xs font-bold px-1.5 py-0.5 rounded shadow-md z-30 flex items-center gap-1">
                  {devLevel === 5 ? "🏨" : `🏠 ×${devLevel}`}
                </div>
              )}
              ...
              {isMortgaged && (
                <>
                  ...
                  <div className="absolute inset-0 flex items-center justify-center z-30 pointer-events-none">
                    <span className="text-red-300/90 text-xl md:text-2xl font-black rotate-[-18deg] tracking-widest drop-shadow-[0_2px_8px_rgba(0,0,0,0.9)]">
                      MORTGAGED
                    </span>
                  </div>
                </>
                ...
                {playersHere.map((player) => {
                  ...
                  return (
                    <motion.span
                      ...
                      className={`
                        text-2xl md:text-3xl lg:text-4xl drop-shadow-lg border-2 rounded-full p-0.5
