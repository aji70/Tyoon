...
const GameRoom = () => {
  ...
  const gameRoomLink = typeof window !== 'undefined' ? window.location.href : 'https://gameroom10qd.io/rok';
    ...
    <>
      ...
      >
        <div className="w-full h-full flex flex-col gap-3">
          ...
          <button onClick={toggleSidebar} className="text-[#869298] hover:text-[#F0F7F7] lg:hidden" aria-label="Toggle sidebar">
            {isSidebarOpen ? <ChevronRight className="w-5 h-5" /> : <PiChatsCircle className="size-[25px]" />}
          </button>
          <div className={`w-full flex justify-between items-center ${!isSidebarOpen && 'hidden'}`}>
            ...
            {/* Settings button */}
            <button className="text-[#869298] hover:text-[#F0F7F7] bg-[#0B191A] size-[32px] rounded-full flex justify-center items-center cursor-pointer" aria-label="Open settings">
