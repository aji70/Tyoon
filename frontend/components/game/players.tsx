...
export default function GamePlayers({
  ...
  const rentPrice = useCallback(
    (id: number) => {
      ...
      return [p?.rent_site_only, p?.rent_one_house, p?.rent_two_houses, p?.rent_three_houses, p?.rent_four_houses, p?.rent_hotel][dev] || 0;
    },
    [properties, developmentStage]
  );

  const sortedPlayers = useMemo(() =>
    [...(game?.players ?? [])].sort((a, b) => (a.turn_order ?? 99) - (b.turn_order ?? 99)),
    [game?.players]
  );
  ...
  const startTrade = (p: Player) => isNext && setTradeModal({ open: true, target: p });
  ...
  const handleMortgage = async (id: number) => {
    ...
    try {
      ...
      if (res.data?.success) toast.success("Property mortgaged");
      else toast.error(res.data?.message || "Cannot mortgage");
    } catch (err: any) {
      toast.error(err.response?.data?.message || "Mortgage failed");
    }
  };
  ...
  const handleUnmortgage = async (id: number) => {
    ...
    try {
      ...
      if (res.data?.success) toast.success("Mortgage paid off");
      else toast.error(res.data?.message || "Cannot unmortgage");
    } catch (err: any) {
      toast.error(err.response?.data?.message || "Unmortgage failed");
    }
  };
  ...
  return (
    <aside className="w-80 h-full bg-gradient-to-b from-[#0a0e17] to-[#1a0033] border-r-4 border-cyan-500 shadow-2xl shadow-cyan-500/50 overflow-y-auto relative">
      ...
      <div className="p-4 space-y-6">
        {/* Header */}
        <motion.h2
          animate={{ textShadow: ["0 0 10px #0ff", "0 0 20px #0ff", "0 0 10px # 10px #0ff"] }}
        ...
        {sortedPlayers.map(p => {
          ...
          return (
            <motion.div
              ...
              className={`p-4 rounded-xl border-2 transition-all ${isTurn ? "border-cyan-400 bg-cyan-900/40 shadow-lg shadow-cyan-400/60" : "border-purple-800 bg-purple-900/20"}`}
            >
              <div className="flex justify-between items-center">
                ...
                <div className="text-xl font-bold text-yellow-400 flex items-center gap-2">
                  <span className="text-xl">COIN</span> ${p.balance.toLocaleString()}
                </div>
              </div>
              {canTrade && (
                <motion.button
              ...
              >
                {my_properties.map((prop, i) => (
                  ...
                  >
                    ...
                    {isMortgaged(prop.id) && <div className="text-red-500 text-xs mt-1 font-bold animate-pulse">MORTGAGED</div>}
                  </motion.div>
                ))}
              </motion.div>
            )}
        ...
        {isNext && selectedProperty && (
          ...
          >
            ...
            >
              ...
              <h3 className="text-3xl font-bold text-cyan-300 text-center mb-6 pr-8">
                {selectedProperty.name}
              </h3>
              <div className="grid grid-cols-2 gap-4">
                <button onClick={() => { handleDevelopment(selectedProperty.id); setSelectedProperty(null); }} className="py-4 bg-green-600 rounded-xl font-bold text-white shadow-lg hover:shadow-green-500/50">BUILD</button>
...
function TradeModal({
  ...
  const PropertyCard = ({ prop, isSelected, onClick }: { prop: Property; isSelected: boolean; onClick: () => void }) => (
    ...
    >
      ...
      <div className="text-xs font-bold text-cyan-200 text-center leading-tight">
        {prop.name}
      </div>
    </div>
  );
  ...
  return (
    ...
    >
      ...
      >
        <button
          onClick={onClose}
          className="absolute top-4 right-6 text-4xl text-red-400 hover:text-red-300 transition z-10"
        >
          X
        </button>
        ...
        <div className="flex justify-center gap-8 mt-12">
          <button
            onClick={onClose}
            className="px-12 py-5 bg-gray-800 rounded-xl font-bold text-2xl text-gray-300 hover:bg-gray-700 transition"
          >
            CANCEL
          </button>
          <button
