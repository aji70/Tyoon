...
const TradeProposalModal = ({
  ...
  const { aiName, property, cash, targetPlayer } = proposal;
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-gray-800 p-6 rounded-lg shadow-xl">
        <h2 className="text-xl font-bold mb-4">Trade Proposal from {aiName}</h2>
        <p className="mb-4">Offers ${cash} for your {property.name}</p>
...
const AiBoard = ({
  ...
  // NEW: AI phase and trade states
  const [aiPhase, setAiPhase] = useState<'start' | 'building1' | 'trading' | 'building2' | 'rolling' | 'done' | null>(null);
 ...
 const ROLL_DICE = useCallback(async (forAI = false) => {
  ...
  // NEW: AI pre-turn actions trigger
  useEffect(() => {
    if (!isAITurn || roll || isRolling || actionLock || aiPhase !== null) return;
    setAiPhase('start');
  }, [isAITurn, roll, isRolling, actionLock, aiPhase]);
  // NEW: AI phase handler
  useEffect(() => {
    if (!isAITurn || !currentPlayer || aiPhase === null || aiPhase === 'done') return;
    const runPhase = async () => {
      if (aiPhase === 'start') {
        await aiHandleBuilding(currentPlayer, game_properties, properties, game, showToast);
        setAiPhase('trading');
      } else if (aiPhase === 'trading') {
        const opportunity = findTradeOpportunity(currentPlayer, game_properties, properties, players);
        if (opportunity) {
          setTradeProposal(opportunity);
          setShowTradeModal(true);
          // Phase pauses until modal resolves
        } else {
          setAiPhase('building2');
        }
      } else if (aiPhase === 'building2') {
        await aiHandleBuilding(currentPlayer, game_properties, properties, game, showToast);
        setAiPhase('rolling');
      } else if (aiPhase === 'rolling') {
        ROLL_DICE(true);
        setAiPhase('done');
      }
    };
    runPhase();
  }, [aiPhase, isAITurn, currentPlayer, game_properties, properties, game, showToast, ROLL_DICE, players]);
  ...
  // NEW: Trade accept/reject handlers
  const handleTradeAccept = async () => {
    if (!tradeProposal || !currentPlayer) return;
    const { targetPlayer, property, cash } = tradeProposal;
    try {
      await apiClient.post("/game-properties/trade", {
        buyer_id: currentPlayer.user_id,
        seller_id: targetPlayer.user_id,
...
// NEW: Helper function to find trade opportunity
const findTradeOpportunity = (
  ...
  const aiAddress = player.address;
  const reserve = 400;
  for (const [color, groupIds] of Object.entries(MONOPOLY_STATS.colorGroups)) {
    if (["railroad", "utility"].includes(color)) continue;
    const group = groupIds as number[];
    const ownedIds = group.filter((id) =>
      gameProperties.find((gp) => gp.property_id === id && gp.address === aiAddress)
    );
    if (ownedIds.length === group.length - 1) {
      const missingId = group.find((id) => !ownedIds.includes(id))!;
      const gp = gameProperties.find((gp) => gp.property_id === missingId);
      if (gp && gp.address && gp.address !== aiAddress && !gp.mortgaged) {
        const owner = allPlayers.find((p) => p.address === gp.address);
        if (
          owner &&
          !owner.username.toLowerCase().includes("ai_") &&
          !owner.username.toLowerCase().includes("bot")
        ) {
          const property = allProperties.find((p) => p.id === missingId)!;
          const offerMultiplier = 1.5;
          const cashOffer = Math.floor(property.price! * offerMultiplier);
          if ((player.balance ?? 0) > cashOffer + reserve) {
            return {
              aiName: player.username,
              property,
              cash: cashOffer,
              targetPlayer: owner,
            };
...
// NEW: Helper function to handle AI building houses
const aiHandleBuilding = async (
  player: Player,
  gameProperties: GameProperty[],
  allProperties: Property[],
  game: Game,
  ...
  for (const [color, groupIds] of Object.entries(MONOPOLY_STATS.colorGroups)) {
    if (["railroad", "utility"].includes(color)) continue;
    const group = groupIds as number[];
    const groupProps = group.map((id) => ({
      prop: allProperties.find((p) => p.id === id)!,
      gp: gameProperties.find((gp) => gp.property_id === id),
    }));
    const ownedGps = groupProps.filter(
      ({ gp }) => gp && gp.address === aiAddress && !gp.mortgaged
    );
    if (ownedGps.length === group.length) {
      const devs = ownedGps.map(({ gp }) => gp!.development ?? 0);
      const minDev = Math.min(...devs);
      if (minDev >= 4) continue;
      const houseCost = ownedGps[0].prop.cost_of_house ?? 0;
      const maxLevels = 3 - minDev; // Aim for 3 houses
      const affordableLevels = Math.floor(
        ((player.balance ?? 0) - reserve) / (houseCost * group.length)
      );
      const levelsToAdd = Math.min(maxLevels, affordableLevels);
      if (levelsToAdd <= 0) continue;
      for (let level = 0; level < levelsToAdd; level++) {
        // Sort by current development to build evenly
        const sortedGps = [...ownedGps].sort(
          (a, b) => (a.gp!.development ?? 0) - (b.gp!.development ?? 0)
        );
        for (const { gp, prop } of sortedGps) {
          try {
            await apiClient.post("/game-properties/build-house", {
              user_id: player.user_id,
              game_id: game.id,
              property_id: prop.id,
            });
            showToast(`AI built a house on ${prop.name}`, "success");
          } catch {
            showToast(`Failed to build house on ${prop.name}`, "error");
          }
          await new Promise((r) => setTimeout(r, 500)); // Short delay for animation/feedback
        }
      }
    }
  }
